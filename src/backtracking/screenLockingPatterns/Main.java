...
public class Main {
    private static final Set<Character> allPossible = new HashSet<>(List.of('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I'));
    private static final Map<Character, Set<Character>> store = initStore();
    private static Map<Character, Set<Character>> initStore() {
        final Map<Character, Set<Character>> store = new HashMap<>();
    ...
    private static HashMap<Character, HashMap<Character, Character>> getUnreachabilityMap() {
    ...
    private static HashMap<Character, Boolean> initVisited() {
        HashMap<Character, Boolean> visited = new HashMap<>();
        for (char ch : allPossible) visited.put(ch, false);
        return visited;
    }


    public static int calculateCombinations(char startPosition, int patternLength) {
        if (patternLength > 9 || patternLength <= 0) return 0;

        return dfs(startPosition, patternLength, 1, initVisited());
    }
    ...
    private static Set<Character> findReachableUnreachable(HashMap<Character, Boolean> visited, char from) {
        HashMap<Character, HashMap<Character, Character>> map = getUnreachabilityMap();
