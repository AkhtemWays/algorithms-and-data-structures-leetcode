package arrays.RepeatedDNASequences;

import java.util.*;

public class Main {
    public static void main(String[] args) {
        System.out.println(findRepeatedDnaSequences("AABAACBAA"));
    ...
    public static List<String> findRepeatedDnaSequences(String s) {
        ...
        // rolling hash parameters: base
        int base = 4, aL = (int)Math.pow(base, L);
        ...
        for (int start = 0; start < n - L + 1; ++start) {
            ...
            if (start != 0)
                h = h * base - nums[start - 1] * aL + nums[start + L - 1];

                // compute hash of the first sequence in O(L) time
            else
                for(int i = 0; i < L; ++i) h = h * base + nums[i];
